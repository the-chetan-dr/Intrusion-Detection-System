...
class AdvancedAnomalyDetector:
    ...
    def add_event(self, feature_vector):
        ...
        if self.model is None:
            logging.info("Model not trained yet — accumulating samples...")
            return

        prediction = self.model.predict([feature_vector])
        if prediction[0] == -1:
            logging.warning("⚠️ Anomaly detected: unusual event pattern!")
